...
namespace WOPR.Services
{
	public class SimulationService
	{
		...
		public async void GetDataFromSave(string SaveFolderPath, string DataPath, string InfrastructureDataPath)
        {
			...
			Map map = new Map(SaveFolderPath+"\\"+saveFile.FullName);
        ...
        {
			//system here
			var GalacticObject = _context.GalacticObjects.FirstOrDefault(g => g.GalacticObjectId == system.GalacticObjectGameId);
		...
		public BabelDatabase.Planet GetPlanet(DPSSimulation.Classes.Planet planet, BabelDatabase.Empire empire, BabelDatabase.GalacticObject galacticObject)
        {
            ...
            {
				NewPlanet.Districts.Add(GetDistrict(district, NewPlanet));
            }
		...
		public BabelDatabase.District GetDistrict(DPSSimulation.Classes.District district, BabelDatabase.Planet planet)
        {
			BabelDatabase.District NewDistrict = _context.Districts.FirstOrDefault(d => d.DistrictId == district.DistrictId);
			if(NewDistrict == null)
            {
        ...
        {
			BabelDatabase.Building NewBuilding = _context.Buildings.FirstOrDefault(b => b.BuildingId == building.BuildingId);
			if(NewBuilding == null)
            {
